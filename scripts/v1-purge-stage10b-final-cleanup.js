...
function replaceFunction(text, name, replacement) {
  ...
  requireCount(text, marker, 1, `function ${name}`);
  const start = text.indexOf(marker);
  const open = text.indexOf("{", start);
  if (open < 0) throw new Error(`${name}: function body not found.`);
...
try {
  ...
  // 1. Game-shell contract becomes current-only instead of classifying historical V1 records.
  ...
  shell = replaceFunction(shell, "actionPhaseVersion", `function actionPhaseVersion() {\n    return DEFAULT_ACTION_PHASE_VERSION;\n  }`);
  ...
  // 3. Shell tests assert the one current Action Phase contract.
  ...
  shellTest = replaceAllExpected(
    ...
    2,
    "progressed shell V1 assertions"
  );
  ...
  // 4. Route browser tests describe a single current renderer/version.
  ...
  // 5. Route runtime fixture advertises only the current Action Phase version.
  let runtimeSeq = read("scripts/test-v2-route-runtime-sequences.js");
  runtimeSeq = replaceExact(
    runtimeSeq,
  ...
  // 6. Remove deleted Encounter client/state buckets from surviving token support modules.
  ...
  // 7. Current Action-operation tests retain shared infrastructure but drop deleted locations/sessions.
  let actionOps = read("scripts/test-action-operation-contract.js");
  actionOps = replaceExact(
    actionOps,
  ...
  actionOps = replaceExact(
    ...
    "Action-operation phase-block title"
  );
  ...
  ]) actionOps = removeExactLine(actionOps, staleLine, `stale Action-operation assertion: ${staleLine}`);
  for (const marker of ["startHiddenGrottoSession", "startEncounterSession", "encounter-session-closed", "hidden-grotto-choice-complete", "V1 Action operation"]) {
    if (actionOps.includes(marker)) throw new Error(`Action-operation stale marker survived transformation: ${marker}`);
  }
  edits.set("scripts/test-action-operation-contract.js", actionOps);

  // 8. Retire pure V1 balance/import tooling and their package scripts.
  ...
  // All transformations have now passed in memory. Only now touch the working tree.
  ...
  // Syntax + focused behavioral validation before committing anything.
  for (const rel of [
    "game-shell-contract.js",
    "token-control-controller.js",
    "token-sandbox-session.js",
    "scripts/test-game-shell-loading.js",
    "scripts/test-v2-route-browser-mount.js",
    "scripts/test-v2-route-runtime-sequences.js",
