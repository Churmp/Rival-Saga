...
const missingRulingById = Object.freeze({
  ...
  "drizzle-field": "Define the exact Drizzle reward trigger: whose real Token consumption is copied, which player receives it, and the reward checkpoint. The canonical inventory-copy mutation is ready."
});
...
function supportedSurfaces(definition) {
  ...
  if (definition.id === "revenge") return "Post-payout optional offer; immutable brought snapshot; exact two-Pokemon and optional held-Item choice; decline; atomic resolver; History; refresh/undo/sandbox infrastructure";
  if (definition.id === "move-deleter") return "Live Referee canonical move selector; next-Gym global restriction; Teambuilder/validator enforcement; History; refresh/phase expiration/undo/sandbox infrastructure";
  ...
  if (definition.id === "lingering-aroma") return "Live Referee exact ongoing-effect selector; linked replacement; confirmed declaration-cost transfer; expiration/undo/sandbox infrastructure";
  ...
  if (definition.id === "knock-off-curse") return "Live Referee exact Pokemon/resource selector; exact Item/TM destruction; final-TM Team Revision request; atomic undo";
  ...
  if (definition.id === "follow-me") return "Live Referee corresponding-target redirect; parent-gated Gym relationship; real inventory-copy runtime; refresh/duplicate prevention";
  if (definition.id === "ditto-token") return "Live Referee searchable canonical Token picker; exact inventory transformation; no immediate activation; undo/duplicate prevention";
  ...
  if (definition.family === "Field") return "Live Referee declaration; active Field record; replacement; History; persistence; undo/sandbox infrastructure";
...
const lines = [
  ...
  "",
  `Generated from the canonical ${contract.list.length}-Token contract on 2026-07-29. Contract revision: \`${contract.contractDefinitionRevision || contract.list[0]?.contractDefinitionRevision || "unknown"}\`.`,
  "",
  "## Runtime Truth",
  "",
  ...
  "",
  "The shared semantic gate opens ordinary Control during idle Action destination decisions, Team Building, Shopping, and after Battle Payout reaches a terminal result. It closes during active location operations, interaction chains, required choices, Team Lock, Sabotage, Team Preview, Rival Battles, and unfinished payout. The server-authoritative pre-destination claim/destination race is unchanged; a withdrawn or resolved interruption returns to the same Action player and Action number.",
  "",
  "Focused evidence: `PD-RUNTIME-001` through `PD-RUNTIME-012`, `PD-SERVER-001` through `PD-SERVER-006`, and production browser scenario `BROWSER-009`.",
  "",
  "## Catalog",
  "",
...
lines.push(
  ...
  "- Category labels, Tickets, Electric Field, Grassy Field, and other legacy/container records are not part of the 53 activatable contracts."
);
