...
Shell and game requests resolve through one runtime API origin. A stale remembered game or profile is cleared narrowly and returns the user to the authoritative list; it never creates a replacement record. Read-only list/open routes do not compact or rewrite authoritative files. Isolated `codex-*` and `browser-smoke` records remain on disk but are excluded from the normal game library.
...
Client-side `localStorage` persistence is intentionally queued for normal `saveState()` calls so quick UI interactions collapse into one snapshot write. The queue flushes when the tab is hidden or closed. Backend state pushes remain separately debounced, so confirmed gameplay changes still sync after the local UI has responded.
...
The reminder import reads supported-game leveling-up tables, excludes Let's Go Pikachu/Eevee and Legends: Z-A sections, and maps separate form tables before updating the shared browser dataset.
...
Held-item descriptions use the smaller generated `item-reference-data.js` bundle so Shop and Teambuilder item help does not force the full Pokemon build dataset to load. Refresh it with `scripts/import-pokeapi-item-data.js`. `shop-choice-data.js` is the canonical source for bundled shop shelves such as Berries, Weather Rocks, Z-Crystals, and unlisted Mega Stones. A shelf name is not an owned held item: the selected concrete option is what enters the cart and inventory. Legacy unresolved shelf entries are excluded from legal held-item choices.
...
- A TM-style move compatible with no more than five distinct PokeAPI species identities is promoted into those Pokemon's Natural movepools and removed from the TM shop. Alternate forms and Mega Evolutions share their underlying species identity; actual evolved species remain distinct. The generated record preserves former learn methods and exact compatible forms for auditing.
...
- `getGameCornerTokenTier(tokenName)`: compatibility helper that converts a Game Corner Ticket name into the corresponding acquisition family.
